...
export async function notionQuery(databaseId) {
    ...
    try {
        ...
        const anyTitleProp = Object.values(props).find((p) => p && p.type === "title");
        let extractedTitle = "(no title)";
        const titleArray = Array.isArray(anyTitleProp?.title)
            ? anyTitleProp.title
            : [];
