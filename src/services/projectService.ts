...
type ProjectPayload = {
    title?: string;
    id?: string;
    client?: string;        // nome cliente per relation
...
async function findProjectByNameOrId(term: string) {
...
async function findClientByName(term: string) {
...
export async function updateProject(payload: ProjectPayload) {
...
export async function deleteProject(payload: ProjectPayload) {
    const matchTerm = payload.id || payload.title;
